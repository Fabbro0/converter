...
/** RTF -> TXT / PDF, via RtfStripper's best-effort control-word stripping. */
class RtfConverter : FileConverter {

    override fun canConvert(from: FileFormat, to: FileFormat): Boolean =
        from == FileFormat.RTF && (to == FileFormat.TXT || to == FileFormat.PDF)

    override fun possibleOutputs(from: FileFormat): Set<FileFormat> =
        if (from == FileFormat.RTF) setOf(FileFormat.TXT, FileFormat.PDF) else emptySet()
    ...
    override suspend fun convert(
        ...
        onProgress(0.2f)
        val plain = RtfStripper.toPlainText(input.sourceFile.readText(Charsets.UTF_8))
        onProgress(0.4f)
        ...
        return when (targetFormat) {
            ...
            else -> ConversionResult.Failure("Formato di destinazione non supportato per RTF")
        }
    }
}
